...
The repository's Pages source must be **GitHub Actions**. [`.github/workflows/deploy-pages.yml`](../.github/workflows/deploy-pages.yml) verifies the frontend, builds the dashboard, `/history/`, `/methodology/`, and `/docs/` into `dist/`, and deploys after successful CI for a code commit, after a successful snapshot sync, or after a finalized CertStream run committed changed public collection health. A collector workflow may report failure after committing its bounded failure record; that trusted data-only completion is still eligible to publish the health change. These paths use GitHub's `workflow_run` event because publisher commits made with the workflow token do not recursively trigger push workflows. Every path runs the same frontend checks before deployment. A changed-path gate rejects any current `main` revision containing code or configuration not validated by the triggering run. Successful hourly synchronization normally advances the public success heartbeat and therefore creates a deployment even when the signal set is unchanged; archive-only URLScan, CT-search, and context-state commits wait for the next snapshot sync.

[`sync-radar.yml`](../.github/workflows/sync-radar.yml) validates the configured inputs each hour. Every successful run advances `lastSuccessfulSyncAt` in the live snapshot, even when the observations are unchanged; `generatedAt` advances only for a material data or source-state change. The same validated signal list is projected into the observation and reviewed STIX files. Persisting the snapshot, shards, both STIX projections, history, optional signal details, aggregate health/change/relation views, schemas, checksums, and manifest in one Git commit keeps the static distributions aligned. Each regular file is first replaced through a temporary file; the Git commit is the atomic repository publication boundary. Live-snapshot retention and sharp-drop protection compare against the actual previous publication rather than an artifact-only copy. The Pages job has no collector credentials and never changes data.

The deploy freshness gate includes the STIX path so a STIX-only correction cannot be mistaken for an unchanged snapshot. The production artifact gate requires `dist/data/radar.stix.json`, while the static verifier checks its 2 MiB cap, STIX structure, raw-domain safety boundary, and exact correspondence to `radar.json`. The feed is served as an ordinary static JSON download. It is not a TAXII endpoint and needs no credential or repository variable.
...
| `sync-radar.yml` | `17 * * * *` and manual dispatch | Persistent live and STIX 2.1 snapshots, per-signal sidecars, candidate history, and compacted history summary | Optional HECAVEX secrets; repository contents write |
...
| `public/data/` synchronized JSON, schemas, and checksums | Snapshot synchronization | Static dashboard, STIX, history, pipeline aggregates, changes, relations, manifest, and integrity files |
