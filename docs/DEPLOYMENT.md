...
The repository's Pages source must be **GitHub Actions**. [`.github/workflows/deploy-pages.yml`](../.github/workflows/deploy-pages.yml) verifies the frontend, builds the dashboard, `/history/`, `/methodology/`, and `/docs/` into `dist/`, and deploys after successful CI for a code commit, after a successful snapshot sync that changed live/STIX/history/detail data, or after a finalized CertStream run committed changed public collection health. A collector workflow may report failure after committing its bounded failure record; that trusted data-only completion is still eligible to publish the health change. These paths use GitHub's `workflow_run` event because publisher commits made with the workflow token do not recursively trigger push workflows. Every path runs the same frontend checks before deployment. A changed-path gate rejects any current `main` revision containing code or configuration not validated by the triggering run. No-change data runs do not deploy redundant content, and URLScan archive-only commits wait for the next snapshot sync.

[`sync-radar.yml`](../.github/workflows/sync-radar.yml) validates the configured inputs each hour. Every successful run advances `lastSuccessfulSyncAt` in the live snapshot, even when the observations are unchanged; `generatedAt` advances only for a material data or source-state change. The same validated signal list is projected into `public/data/radar.stix.json`. Persisting `public/data/radar.json`, `public/data/radar.stix.json`, `public/data/history.json`, `public/data/signals/`, and `data/history/` in one Git commit keeps the current view, STIX observables, optional detail declarations, and reproducible observation trail aligned. Each regular file is first replaced through a temporary file; the Git commit is the atomic repository publication boundary. Live-snapshot retention and sharp-drop protection compare against the actual previous publication rather than an artifact-only copy. The Pages job has no collector credentials and never changes data.
