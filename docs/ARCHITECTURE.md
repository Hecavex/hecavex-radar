...
URLScan is an optional passive enricher. If its credential is absent, the hunter exits successfully with an explicit skip message and does not mutate its archive. That state is not treated as a benign result, a collector failure, or a reason to remove or downgrade CertStream candidates.
