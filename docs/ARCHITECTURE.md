...
CertStream and URLScan collection write daily archives. `python -m hecavex_radar.sync` validates those archives together with an optional HECAVEX export, then merges, limits, sorts, and atomically replaces `public/data/radar.json`. The scheduled snapshot writer commits that file so the next run has the real prior-publication baseline; Pages only builds the committed result. The browser has no backend API, login, or database. HECAVEX feed URLs require HTTPS in production and cannot contain credentials or an explicit port. For local development only, HTTP is accepted on `localhost`, `127.0.0.1`, or `::1` at the default port.
