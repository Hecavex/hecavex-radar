...
3. A narrow exception covers a long alias joined directly to one suspicious prefix or suffix, such as `securerevolut` or `revolutlogin`. The remaining text must exactly equal a word in the suspicious-word set; arbitrary substrings such as `revolut` in `revolution` do not match.
4. One-edit matching is opt-in. It applies only to a single-word alias listed in that brand's `fuzzyAliases`, with suspicious context in the same label or punycode evidence. Digit-bearing aliases must preserve their digits.
