...
`public-decisions.json` is the only analyst-review artifact consumed by the production `radar.hecavex.com` pipeline. It contains bounded, defanged suppressions and manually observed candidates that still pass the current public brand matcher. It contains no analyst identity, private note, credential, or raw evidence.
