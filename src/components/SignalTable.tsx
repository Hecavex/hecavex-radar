import { Camera, Check, ChevronLeft, ChevronRight, Copy, SearchX } from "lucide-react";
...
function Confidence({ value }: { value: number }) {
  ...
  return (
    <span className={`confidence ${level}`} aria-label={`${value}% confidence`}>
              ...
              <tr key={signal.id}>
                ...
                <td className="capture-cell">
                  {signal.screenshotUrl ? (
                    <button type="button" onClick={() => setCapture(signal)} aria-label={`View capture for ${signal.domain}`}>
                      <Camera aria-hidden="true" />
                    </button>
                  ) : <span aria-label="No capture available">—</span>}
                </td>
              </tr>
            ))}
          </tbody>
