...
The Apache License 2.0 covers the HECAVEX Radar software and original documentation. It does not relicense third-party data, trademarks, screenshots, or source material.

Published records use only the source labels CertStream, URLScan, and HECAVEX, and the snapshot includes those source homepages. Third-party discovery lists are transient search inputs: they cannot create a dashboard row, are not copied into the archives, and are never relabeled as HECAVEX. Deployers remain responsible for the current access, attribution, redistribution, and rate-limit terms of every input they enable.
...
- **URLScan:** report metadata, screenshots, and hashes remain subject to URLScan's terms and depicted-site rights. Before public or commercial deployment, confirm that the account and plan permit the intended automated access, display, and redistribution; an API key alone does not grant those rights.
